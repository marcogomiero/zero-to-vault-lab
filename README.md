...
-   A new top-level script `lab-menu.sh` has been introduced to manage both **Vault** and **OpenBao** labs from a single entry point (currently in BETA)
...
OpenBao Lab (via `bao-lab-ctl.sh`)
----------------------------------

This section tracks the main milestones and features introduced in different versions of the `zero-to-openbao-lab` project.

### Key Features ✨

This script automates the creation and management of a single-instance OpenBao environment with the following configurations:

-   **File Backend Only:** OpenBao is configured to use the `file` storage backend by default, providing a simple and persistent storage solution for your lab data. 🗄️

-   **Automated OpenBao Installation:** The `bao-lab-ctl.sh` script automatically downloads and sets up the **latest stable OpenBao binary**. If a recent version is already present, it reuses it to save you time. ⏱️

-   **Robust Process Management:** The `bao-lab-ctl.sh` script tracks the OpenBao server's Process ID (PID) for a more reliable and targeted shutdown and restart. ✅

-   **Pre-configured Environment:**

    -   OpenBao is initialized and "unsealed" automatically. No need to worry about unseal keys (for the lab, of course!).
...
-   **Audit Device:** A file type audit device is enabled, with logs directed to `/dev/null` by default (configurable). You can see what's happening in your OpenBao. 📝

-   **Test Secrets:** Some example secrets are populated to facilitate initial testing and let you start experimenting with OpenBao right away. 🧪

-   **Clear Output:** Upon setup completion, the script provides all essential information to access and interact with your OpenBao environment (URL, token, AppRole credentials, example commands). Everything at your fingertips! 📋

-   **Flexible Output:** Supports colored terminal output by default, but can be disabled for scripting or logging purposes. 🌈

### How to Use the OpenBao Lab ▶️

Follow these simple steps to start and configure your OpenBao environment:

1.  **Clone the Repository**

    Open your terminal and clone the project (or ensure you are in the correct directory if you have already cloned):

    ```
    git clone https://github.com/marcogomiero/zero-to-hashicorp-labs.git # Make sure to update your repository URL if different
    cd zero-to-hashicorp-labs
...
2.  **Run the Control Script**
    ...
    The `bao-lab-ctl.sh` script is your main entry point. It will handle downloading OpenBao (if needed), starting it, initializing it, unsealing it, and configuring all secrets engines and authentication methods.

    ```
    ./bao-lab-ctl.sh [COMMAND] [OPTIONS]

    ```

    **Available Commands and Options for `bao-lab-ctl.sh`:**

    -   **`start`** (Default Command): Configures and starts the OpenBao lab.

        -   No options: Default behavior. The script will detect an existing OpenBao lab and ask for confirmation to clean it up.

        -   `-c, --clean`: Forces a clean setup, removing any existing OpenBao data in `$BAO_DIR` before starting, without prompting.

        -   `-b, --base-directory <path>`: Specify the base directory for the OpenBao lab. Overrides the default `BASE_DIR`.

    -   **`stop`**: Stops the running OpenBao server gracefully.

    -   **`restart`**: Stops the OpenBao server and then starts it again.

    -   **`status`**: Checks and displays the current operational status of the OpenBao server (initialized, sealed, active node, etc.).

    -   **`cleanup`**: Stops any running OpenBao instances and removes all lab data and configuration files.
    ...
    **Script Behavior (when running `start`):**
    ...
    -   If it detects a previous OpenBao environment (`bao-lab/`), it will ask you if you want to clean it up and start from scratch (`y/N`).

        -   Answering `y` (or `Y`) will perform a complete cleanup of the environment, followed by a fresh setup.

        -   Answering `N` (or anything else) will attempt to restart OpenBao with existing data and unseal it if necessary (assuming keys are present). In this case, existing configurations will be reapplied idempotently.

    -   If no previous environment is detected, it proceeds with a fresh setup.

3.  **Access OpenBao 🔐**

    After running `bao-lab-ctl.sh start`, you will receive a very clear summary of access information directly in your terminal.

    **From CLI (Command Line Interface):** To configure your shell and interact with OpenBao, copy and paste the commands suggested by the script's final output. They will be similar to these:

    ```
    export BAO_ADDR="https://127.0.0.1:8200"
    export BAO_TOKEN="root"
    export BAO_SKIP_VERIFY=true # Necessary for self-signed certs in lab

    ```

    Once environment variables are set, you can use the `bao` command to interact with your instance. For example:

    ```
    bao status
    bao kv get secret/test-secret

    ```

    **From UI (User Interface):** Open your favorite browser and navigate to: `https://127.0.0.1:8200`. Use "root" as the token to log in to the graphical OpenBao interface. Easy, right? ✨

    **Troubleshooting & Logs:** If OpenBao fails to start or behave as expected, check the detailed logs at: `$BAO_DIR/bao.log`.
...
### OpenBao Lab (`bao-lab-ctl.sh`)

**v1.2.0 - Unified Control & Enhanced Management (Current)**

This version significantly enhances the lab environment management by consolidating all operations into a single control script (`bao-lab-ctl.sh`) and introducing new commands for a more streamlined workflow.

-   **Unified Control Script:** All lab management commands (`start`, `stop`, `restart`, `status`, `cleanup`) are now integrated into a single `bao-lab-ctl.sh` script, simplifying usage and reducing script proliferation. This replaces the need for separate `setup-lab-openbao.sh` and `stop-lab-openbao.sh` scripts.

-   **New Commands:**

    -   `stop`: Gracefully stops the running OpenBao server.

    -   `restart`: Stops and then restarts the OpenBao server.

    -   `status`: Checks and displays the current operational status of the OpenBao server (initialized, sealed, active node, etc.).

    -   `cleanup`: Stops OpenBao and removes all lab data.

-   **Configurable Base Directory:** Added a `-b` or `--base-directory` option to specify the root directory for the OpenBao lab, allowing for greater flexibility in deployment location.

-   **Updated Prerequisite Check:** Added `openssl` to the list of automatically checked and installed prerequisites, as it's now essential for generating TLS certificates.

**v1.1.0 - Initial Release & Core Features**

This version consolidated all core features and enhancements developed for the OpenBao lab environment, providing a robust and easy-to-use setup.

-   **Automated OpenBao Binary Management:** The script automatically downloads the latest stable OpenBao binary. It intelligently checks the local OpenBao binary version against the latest available release and only downloads/updates if necessary, saving time.

-   **Automated Prerequisite Installation:** The script checks for necessary tools (`curl`, `jq`, `tar`, `lsof`) and prompts for automatic installation using the system's package manager if missing.

-   **Basic OpenBao Lab Setup:** Provides an automated setup for a single-instance OpenBao lab with file storage.

-   **Pre-configuration:** Includes automatic initialization, unsealing, basic secrets engine enablement (KV, PKI), authentication method setup (AppRole, Userpass), and test secret population.

-   **Essential Management:** Provided clear instructions and dedicated scripts (`setup-lab-openbao.sh` and `stop-lab-openbao.sh`) for starting, stopping, and cleaning the lab environment.

-   **Robust Process Management:** Enhanced detection and stopping routines for OpenBao processes.

-   **Cross-OS Compatibility Improvements:** Binary download logic was improved for different operating systems (Linux, macOS, Windows/WSL).

-   **Clear and Flexible Output:** Provided clear, colored output to the terminal by default, with an option to disable colors for scripting or logging.
