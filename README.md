...
This lab environment is designed to be easily managed and extended using HashiCorp Terraform, the industry standard for Infrastructure as Code (IaC). After you have successfully deployed and initialized your Vault instance using `setup-lab-vault.sh`, you can use the provided Terraform configurations to further manage Vault resources.

**1. Install Terraform:**
Ensure you have Terraform installed on your system. The `setup-lab-vault.sh` script will check for it and prompt you to install it if missing. Alternatively, you can download it from the [official HashiCorp website](https://www.terraform.io/downloads.html) or use your system's package manager.

**2. Explore Terraform Examples:**
Navigate to the `terraform-examples/` directory within this project:

**3. Authenticate Terraform to Vault:**

Terraform needs to authenticate to your Vault instance. The simplest way for this lab is to use the VAULT_ADDR and VAULT_TOKEN environment variables. Make sure your shell is configured with the root token provided by the setup-lab-vault.sh script:

export VAULT_ADDR="[http://127.0.0.1:8200](http://127.0.0.1:8200)"
export VAULT_TOKEN="root" # Or the token generated by the setup script

**4. Run Terraform:**

Once authenticated, you can apply the example configurations.

terraform init          # Initialize the Terraform working directory
terraform plan          # Review the changes Terraform will make
terraform apply         # Apply the changes to your Vault instance

To remove the resources managed by Terraform, you can use:
terraform destroy

Example Terraform Configurations Provided:

main.tf: Configures the Vault provider and ensures common KV engines are enabled.

policy.tf: Defines a Vault policy (terraform-read-policy) with read capabilities.

secrets.tf: Writes a sample secret to secret/terraform-managed-secret.
